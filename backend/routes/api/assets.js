...
// ✅ CHANGE: Added "/add" to match your frontend fetch call
// URL: POST https://warranty-vault-fullstack.vercel.app/api/assets/add
...
// ✅ GET all assets
// URL: GET https://warranty-vault-fullstack.vercel.app/api/assets
router.get("/", auth, async (req, res) => {
  try {
    const assets = await Asset.find({ user: req.user.id }).sort({ createdAt: -1 });
    res.json(assets);
...
// ✅ DELETE an asset
// URL: DELETE https://warranty-vault-fullstack.vercel.app/api/assets/:id
router.delete("/:id", auth, async (req, res) => {
  try {
    const asset = await Asset.findById(req.params.id);
    if (!asset) return res.status(404).json({ msg: "Asset not found" });
...
// URL: PUT https://warranty-vault-fullstack.vercel.app/api/assets/:id
router.put("/:id", auth, async (req, res) => {
  try {
    let asset = await Asset.findById(req.params.id);
    if (!asset) return res.status(404).json({ msg: "Asset not found" });
