...
export function Dashboard() {
  ...
  // Responsive spacing (UI Constants kept exactly the same)
  const isMobile = breakpoint === 'mobile';
  const isTablet = breakpoint === 'tablet';
  const padding = isMobile ? 'p-3' : isTablet ? 'p-4' : 'p-8';
  const gridGap = isMobile ? 'gap-3' : isTablet ? 'gap-4' : 'gap-6';
  ...
  const handleDeleteAsset = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setAssets(prev => prev.filter(a => (a.id || (a as any)._id) !== id));
  ...
  const handleUpdateAsset = async (updatedAsset: Asset) => {
    // Logic for updating in backend can be added here similarly
    setSelectedAsset(updatedAsset);
    fetchAssets(); 
  };
...
const totalValue = assets.reduce((sum, asset: any) => {
  const currentVal = calculateCurrentValue(
    Number(asset.purchaseCost),
    new Date(asset.purchaseDate),
    Number(asset.depreciationRate || 20)
  );
  return sum + Math.round(currentVal);
}, 0);
  // const filteredAssets = assets.filter((asset) =>
  //   asset.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  // );
  // This logic filters your MongoDB assets in real-time
const filteredAssets = assets.filter((asset) => {
  const matchesSearch = 
    asset.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    asset.manufacturer.toLowerCase().includes(searchQuery.toLowerCase()) ||
