...
import { motion } from 'framer-motion'; // Using framer-motion for stability
import { Package, Calendar, DollarSign, Shield } from 'lucide-react';
import { Asset } from '../types/asset';
import { formatCurrency, formatDate, calculateWarrantyHealth, calculateCurrentValue } from '../utils/calculations';
import { Progress } from './ui/progress';
...
export function AssetCard({ asset, index, onClick }: AssetCardProps) {
   ...
   * Convert MongoDB strings/placeholders into valid Numbers and Dates 
   * to prevent $NaN and 0% errors.
   */
  const purchaseCost = Number(asset.purchaseCost) || 0;
  const depreciationRate = Number(asset.depreciationRate) || 20;
  
  // Ensure we have a valid Date object for calculations
  const purchaseDate = typeof asset.purchaseDate === 'string' 
    ? new Date(asset.purchaseDate) 
    : asset.purchaseDate;

  // Calculate real-time values for the UI
      ...
      <div className="relative z-10">
        ...
        <div className="mb-4">
          ...
          <div className="h-2 bg-white/10 rounded-full overflow-hidden">
            <motion.div 
              initial={{ width: 0 }}
              animate={{ width: `${warrantyHealth}%` }}
              transition={{ duration: 1, ease: "easeOut" }}
              className={`h-full bg-gradient-to-r ${getHealthColor(warrantyHealth)} shadow-[0_0_10px_rgba(0,255,136,0.2)]`}
            />
          </div>
        </div>
