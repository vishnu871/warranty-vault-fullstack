...
/**************** */
// import { useState } from 'react';
// import { motion, AnimatePresence } from 'framer-motion'; // Updated to 'framer-motion' for stability
// import { X, ChevronRight, ChevronLeft, Check } from 'lucide-react';
// import { Button } from './ui/button';
// import { Input } from './ui/input';
// import { Label } from './ui/label';
...
//   isOpen: boolean;
//   onClose: () => void;
//   onRefresh: () => void; // Changed from onAdd to onRefresh to trigger dashboard reload
// }

// const STEPS = ['Basic Info', 'Warranty & Service', 'Review'];

// export function AddAssetModal({ isOpen, onClose, onRefresh }: AddAssetModalProps) {
//   const [currentStep, setCurrentStep] = useState(0);
//   const [formData, setFormData] = useState<AssetFormData>({
//     name: '',
//     category: '',
...
//     // Construct the payload to match your backend model exactly
//     const assetPayload = {
//       name: formData.name,
//       manufacturer: formData.manufacturer,
//       category: formData.category,
...
//       maintenanceTasks: formData.maintenanceTasks
//     };
...
//       if (response.ok) {
//         onRefresh(); // Refresh the dashboard cards
//         handleClose();
//       } else {
...
//                     <div>
//                       <div className="flex items-center justify-between mb-3">
//                         <h3 className="text-lg font-semibold text-white">Maintenance Tasks (Optional)</h3>
//                         <Button
//                           type="button"
//                           onClick={addMaintenanceTask}
//                           size="sm"
...
export function AddAssetModal({ isOpen, onClose, onRefresh }: AddAssetModalProps) {
  ...
  // 2. Updated handleSubmit with Toast and Real API Call
  const handleSubmit = async () => {
    const token = localStorage.getItem('token');
    ...
    try {
      ...
      if (response.ok) {
        // SUCCESS TOAST
        toast.success("Asset Vault Updated", {
        ...
        toast.error("Failed to add asset", {
          description: errorData.msg || "Please check your connection."
        });
      }
    } catch (err) {
              ...
              <div className="flex gap-2">
                {currentStep < STEPS.length - 1 ? (
                  <Button
                    onClick={handleNext}
                    disabled={!canProceed()}
                    className="bg-gradient-to-r from-[#00E5FF] to-[#00B8D4] text-white rounded-xl"
                  >
                    Next
                    <ChevronRight className="w-4 h-4 ml-1" />
                  </Button>
                  ...
                  <Button
                    onClick={handleSubmit}
                    className="bg-gradient-to-r from-[#00FF88] to-[#00CC6F] text-white rounded-xl"
                  >
                    <Check className="w-4 h-4 mr-1" />
                    Add Asset
                  </Button>
